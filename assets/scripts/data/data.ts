// 跨文件数据引用（原项目全局变量，模块化后需显式 import）
import { ITEM_DATA } from './data_item';
import { SCIENCE_DATA } from './data_studio';
import { EVENT_DATA } from './data_event';
import { PLACE_DATA } from './data_place';
import { clone } from '../core/utils';

// 统一 re-export，使 data.ts 成为所有数据的入口（外部统一从 '../data/data' 导入）
export { ITEM_DATA } from './data_item';
export { MAKE_DATA, ALCHEMY_DATA, MAGIC_DATA, SCIENCE_DATA } from './data_studio';
export { EVENT_DATA } from './data_event';
export { MST_DATA, PREFIX_DATA, UPPER_CHANCE } from './data_mst';
...
export const NIGHT_BEGIN = 22;
export const NIGHT_END = 2;
export const SEASON_CIRCLE = 15;
export const TRADE_MUL = 0.75;
export const COLOR = {
    BLACK:'#222',
    GREEN:'#74AB6A',
    BLUE:'#62BBBE',
...
export const BUILDING_UPDATE_DATA = {
    cookerUpdate:{
        cooker_1:{
            timeNeed:4,
...
export const COOK_DATA = [
    ...
    {name:"jamBread",require:["jam","wheat"]},
    ...
    {name:"seedBread",require:["seed","wheat"]},
...
//角色状态初始化
export const PLAYER_STATE_INIT = {
    temp:{
        amount:0
    },
    hp:{
...
// 道具耐久度初始化
// 初始为空对象：耐久采用「懒初始化」——装备或使用时再按需写入满耐久。
// 注意：绝不能预置为 0！旧写法会把每个工具初值设 0，导致 durableSaveData
...
for(var attr in PLACE_DATA){
    ...
    if(PLACE_DATA[attr].things){
        PLACE_INIT[attr].things = JSON.parse(JSON.stringify(PLACE_DATA[attr].things));
    }
