...
- **UILabel 自适应策略（2026-08-05 改版，纠偏旧铁律）**：**不传 `width` → `overflow=NONE`（引擎按字体度量真实量字，文字永不裁切，等价 CSS `width:auto`）**；**传 `width` → 固定容器语义**（`overflow=CLAMP` 裁切 / `SHRINK` 缩字，用于按钮/Tab/弹窗行 meta 等需限定宽度的场景）。旧「UILabel 必须传 width 否则被裁」铁律不再成立——现在漏传 width 反而更安全（默认 NONE 不裁切）。仍保留：① 固定容器内文字**显式传 `width`** 以限定换行/裁切边界；② 不传 width 时估算尺寸改用**混合字符宽**（CJK≈1.0×字号、ASCII/数字≈0.6×字号，见 `estimateTextWidth`），让父容器（VStack/HStack/行/面板，无 CSS 回流链）排版尺寸接近真实。同时给 `UILabel` 加了 `string` 读写器（兼容 `.string=` 写法，消除 ProgressOverlay 存量 TS2339）。ModalTab/UIButton/ModalRow/ModalInfoRow/Toast 仍传 width 走固定语义，无回归。
...
- **UILabel 不传 `width` 时默认 `chars*size*0.6` 紧估对汉字严重不足（2026-08-04 ModalInfoRow 标签裁切坑，已于 2026-08-05 根治）**：旧版不传 width 走 `CLAMP` + 0.6 估宽 → 汉字方块字宽≈fontSize 放不下被居中裁切。已改版：不传 width → `NONE` 自适应不裁切 + 估算用**混合字符宽**（CJK≈1.0×字号、ASCII/数字≈0.6×字号）。该坑已通过改 UILabel 内核彻底消除，不再依赖「人肉补 width」铁律。
